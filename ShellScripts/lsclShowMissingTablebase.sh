...
readarray -d '' lsclAllReductions < <(find $lsclRepoDir/Projects/$lsclProjectName/Diagrams/$lsclProcessName/$lsclModelName/$lsclNLoops/Reductions/*/ -type f -name "*.tables" -print0);
...
for i in "${lsclAllReductions[@]}"; do
  ...
  if [ ! -f ${lsclTopoDirName}/tablebase.tbl ]; then
   echo "${lsclTopoName}"
  fi
