...
# ./ShellScripts/lsclImportReductionResults.sh MyProject MyProject MyModel 3 topology1
# ./ShellScripts/lsclImportReductionResults.sh MyProject MyProject MyModel 3 topology1 --force
# ./ShellScripts/lsclImportReductionResults.sh MyProject MyProject MyModel 3 topology1 --epexpand
# ./ShellScripts/lsclImportReductionResults.sh MyProject MyProject MyModel 3 --fromto 1 2
# ./ShellScripts/lsclImportReductionResults.sh MyProject MyProject MyModel 3 --fromto 1 all --force

# Stop if any of the commands fails
set -e

export LSCL_SCRIPT_NAME="lsclImportReductionResults"
export LSCL_SHELL_SCRIPT_NAME="AuxiliaryScripts/lsclAuxImportReductionResults.sh"
...
while [[ ${#} -gt 0 ]]; do
  case ${1} in
    ...
    --kira)
      ...
      echo "${LSCL_SCRIPT_NAME}: Reduction tables generated by KIRA."
      shift
      ;;        
...
if [[ ${LSCL_FLAG_KIRA} -ne 1 ]] ; then
  echo "${LSCL_SCRIPT_NAME}: Reduction tables generated by FIRE."
fi

if [[ ${LSCL_FLAG_KIRA} -ne 1 ]] ; then
  if [[ ${LSCL_FLAG_FORCE} -eq 0 ]] && [[ ${lsclOptFromTo} -ne 1 ]]; then
        if [[ ${LSCL_FLAG_EXPAND_IN_EP} -eq 0 ]]; then
          export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Reductions/${lsclTopologyName}/FireReductionRules.m"
        else
          export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Reductions/${lsclTopologyName}/FireReductionRulesExpanded${LSCL_EP_EXPANSION_ORDER}.m"
        fi     
  fi
else
  if [[ ${LSCL_FLAG_FORCE} -eq 0 ]] && [[ ${lsclOptFromTo} -ne 1 ]]; then
        if [[ ${LSCL_FLAG_EXPAND_IN_EP} -eq 0 ]]; then
          export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Reductions/${lsclTopologyName}/KiraReductionRules.m"
        else
          export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Reductions/${lsclTopologyName}/KiraReductionRulesExpanded${LSCL_EP_EXPANSION_ORDER}.m"
        fi     
  fi
fi
...
if [[ ${lsclOptFromTo} -eq 1 ]] ; then
    ...
    export LSCL_SCRIPT_TO_RUN_IN_PARALLEL="lsclImportReductionResults.sh"
