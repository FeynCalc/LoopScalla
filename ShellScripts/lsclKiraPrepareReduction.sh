...
# ./ShellScripts/lsclKiraPrepareReduction.sh MyProject MyProject MyModel 3 topology1
# ./ShellScripts/lsclKiraPrepareReduction.sh MyProject MyProject MyModel 3 topology1 --force
# ./ShellScripts/lsclKiraPrepareReduction.sh MyProject MyProject MyModel 3 --fromto 1 2
# ./ShellScripts/lsclKiraPrepareReduction.sh MyProject MyProject MyModel 3 --fromto 1 all --force

# Stop if any of the commands fails
set -e

export LSCL_SCRIPT_NAME="lsclKiraPrepareReduction"
export LSCL_SHELL_SCRIPT_NAME="AuxiliaryScripts/lsclAuxKiraPrepareReduction.sh"
...
if [[ ${LSCL_FLAG_FORCE} -eq 0 ]] && [[ ${lsclOptFromTo} -ne 1 ]]; then
      export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Reductions/${lsclTopologyName}/job.yaml"
fi

if [[ ${lsclOptFromTo} -eq 1 ]] ; then
    # Process multiple diagrams in parallel

    export LSCL_SCRIPT_TO_RUN_IN_PARALLEL="lsclKiraPrepareReduction.sh"
