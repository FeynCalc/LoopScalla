...
# ./ShellScripts/lsclAnalyzePoles.sh MyProject MyProject MyModel 3 topology1
# ./ShellScripts/lsclAnalyzePoles.sh MyProject MyProject MyModel 3 topology1 --force
# ./ShellScripts/lsclAnalyzePoles.sh MyProject MyProject MyModel 3 --fromto 1 2
# ./ShellScripts/lsclAnalyzePoles.sh MyProject MyProject MyModel 3 --fromto 1 all --force

# Stop if any of the commands fails
set -e

export LSCL_SCRIPT_NAME="lsclAnalyzePoles"
export LSCL_SHELL_SCRIPT_NAME="AuxiliaryScripts/lsclAuxAnalyzePoles.sh"
...
if [[ ${LSCL_FLAG_FORCE} -eq 0 ]] && [[ ${lsclOptFromTo} -ne 1 ]]; then
      if [[ ${LSCL_FLAG_EXPAND_IN_EP} -eq 0 ]]; then
        export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/Output/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Reductions/${lsclTopologyName}/FireReductionRulesUnphysicalPoles.m"
      else
        export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/Output/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Reductions/${lsclTopologyName}/FireReductionRulesUnphysicalPoles.m"
      fi     
fi
...
if [[ ${lsclOptFromTo} -eq 1 ]] ; then
    ...
    export LSCL_SCRIPT_TO_RUN_IN_PARALLEL="lsclAnalyzePoles.sh"
