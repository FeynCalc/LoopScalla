...
export LSCL_SCRIPT_NAME="lsclFireImportResults"
export LSCL_SHELL_SCRIPT_NAME="AuxiliaryScripts/lsclAuxFireImportResults.sh"
...
if [[ ${LSCL_FLAG_FORCE} -eq 0 ]] && [[ ${lsclOptFromTo} -ne 1 ]]; then
      if [[ ${LSCL_FLAG_EXPAND_IN_EP} -eq 0 ]]; then
        ...
        export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/Output/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Reductions/${lsclTopologyName}/FireReductionRulesExpanded${LSCL_EP_EXPANSION_ORDER}.m"
      fi     
fi
...
if [[ ${lsclOptFromTo} -eq 1 ]] ; then
    ...
    export LSCL_SCRIPT_TO_RUN_IN_PARALLEL="lsclFireImportResults.sh"
