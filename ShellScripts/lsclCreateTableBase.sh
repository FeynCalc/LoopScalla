...
while [[ ${#} -gt 0 ]]; do
  case ${1} in
    ...
    --kira)
      ...
      lsclExtraFormScriptArguments+=("-d LSCLKIRA")
      shift
      ;;   
    #Expansion in ep
    --epexpand)      
...
if [[ ${LSCL_FLAG_KIRA} -ne 1 ]] ; then
  echo "${LSCL_SCRIPT_NAME}: Using reduction tables generated by FIRE."
fi
...
if [[ ${LSCL_FLAG_KIRA} -ne 1 ]] ; then
  if [[ ${LSCL_FLAG_FORCE} -eq 0 ]] && [[ ${lsclOptFromTo} -ne 1 ]]; then
        if [[ ${LSCL_FLAG_EXPAND_IN_EP} -eq 0 ]]; then
          export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Reductions/${lsclTopologyName}/tablebaseFire.tbl"
        else
          export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Reductions/${lsclTopologyName}/tablebaseFireExpanded${LSCL_EP_EXPANSION_ORDER}.tbl"
        fi     
  fi
else
  if [[ ${LSCL_FLAG_FORCE} -eq 0 ]] && [[ ${lsclOptFromTo} -ne 1 ]]; then
        if [[ ${LSCL_FLAG_EXPAND_IN_EP} -eq 0 ]]; then
          export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Reductions/${lsclTopologyName}/tablebaseKira.tbl"
        else
          export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Reductions/${lsclTopologyName}/tablebaseKiraExpanded${LSCL_EP_EXPANSION_ORDER}.tbl"
        fi     
  fi
fi
