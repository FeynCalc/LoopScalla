...
# ./ShellScripts/lsclCreateTablebase.sh MyProject MyProject MyModel 3 topology1
# ./ShellScripts/lsclCreateTablebase.sh MyProject MyProject MyModel 3 topology1 --force
# ./ShellScripts/lsclCreateTablebase.sh MyProject MyProject MyModel 3 --fromto 1 2
# ./ShellScripts/lsclCreateTablebase.sh MyProject MyProject MyModel 3 --fromto 1 all --force

# Stop if any of the commands fails
set -e

export LSCL_SCRIPT_NAME="lsclCreateTablebase"
export LSCL_FORM_SCRIPT_NAME="lsclCreateTableBase.frm"
...
if [[ ${LSCL_FLAG_FORCE} -eq 0 ]] && [[ ${lsclOptFromTo} -ne 1 ]]; then
  ...
  if [[ ${LSCL_FLAG_EXPAND_IN_EP} -eq 0 ]]; then
        export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Reductions/${lsclTopologyName}/tablebase.tbl"
      else
        export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Reductions/${lsclTopologyName}/tablebaseExpanded${LSCL_EP_EXPANSION_ORDER}.tbl"
    fi            
fi
...
if [[ ${lsclOptFromTo} -eq 1 ]] ; then
    ...
    export LSCL_SCRIPT_TO_RUN_IN_PARALLEL="lsclCreateTableBase.sh"
    ...
    ${lsclScriptDir}/lsclTemplateScriptForm.sh ${lsclBasicArguments[@]:0:4} ${lsclTopologyName} ${lsclExtraFormScriptArguments[@]}
fi
