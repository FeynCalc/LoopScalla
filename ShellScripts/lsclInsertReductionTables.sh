...
# Examples:
# /ShellScripts/lsclInsertReductionTables.sh MyProject MyProject MyModel 3 1
# /ShellScripts/lsclInsertReductionTables.sh MyProject MyProject MyModel 3 1 --force
# /ShellScripts/lsclInsertReductionTables.sh MyProject MyProject MyModel 3 --fromto 1 10
# /ShellScripts/lsclInsertReductionTables.sh MyProject MyProject MyModel 3 --fromto 1 all --force

# Stop if any of the commands fails
set -e

export LSCL_SCRIPT_NAME="lsclInsertReductionTables"
...
if [[ ${LSCL_FLAG_FORCE} -eq 0 ]] && [[ ${lsclOptFromTo} -ne 1 ]]; then
      export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/Output/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/Stage2/stage2_dia${lsclDiaNumber}L${lsclNLoops}.res"
fi

if [[ ${lsclOptFromTo} -eq 1 ]] ; then
    # Process multiple diagrams in parallel
    export LSCL_SCRIPT_TO_RUN_IN_PARALLEL="lsclInsertReductionTables.sh"
