...
if [[ ${LSCL_FLAG_KIRA} -ne 1 ]] ; then
  if [[ ${LSCL_FLAG_FORCE} -eq 0 ]]; then
        export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/LoopIntegrals/MasterIntegralMappingsFire.m"
  fi
else
  if [[ ${LSCL_FLAG_FORCE} -eq 0 ]]; then
        export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/LoopIntegrals/MasterIntegralMappingsKira.m"
  fi
fi  
