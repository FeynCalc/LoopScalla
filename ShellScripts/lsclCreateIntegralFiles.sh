...
# ./ShellScripts/lsclCreateIntegralFiles.sh MyProject MyProject MyModel 3 topology1
# ./ShellScripts/lsclCreateIntegralFiles.sh MyProject MyProject MyModel 3 topology1 --force
# ./ShellScripts/lsclCreateIntegralFiles.sh MyProject MyProject MyModel 3 --fromto 1 2
# ./ShellScripts/lsclCreateIntegralFiles.sh MyProject MyProject MyModel 3 --fromto 1 all --force

# Stop if any of the commands fails
set -e

export LSCL_SCRIPT_NAME="lsclCreateIntegralFiles"
export LSCL_FORM_SCRIPT_NAME="lsclCreateIntegralFiles.frm"
export LSCL_FORM_SCRIPT_INPUT_VARIABLE="lsclTopologyName"
echo
echo
...
if [[ ${LSCL_FLAG_FORCE} -eq 0 ]] && [[ ${lsclOptFromTo} -ne 1 ]]; then
      export LSCL_RUN_ONLY_IF_RESULT_FILE_NOT_PRESENT="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/Output/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/LoopIntegrals/Mma/${lsclTopologyName}.m"
fi

export LSCL_CREATE_DIR_IF_NOT_PRESENT_1="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/Output/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/LoopIntegrals/Mma"
export LSCL_CREATE_DIR_IF_NOT_PRESENT_2="${lsclRepoDir}/Projects/${lsclProjectName}/Diagrams/Output/${lsclProcessName}/${lsclModelName}/${lsclNLoops}/LoopIntegrals/Form"
...
if [[ ${lsclOptFromTo} -eq 1 ]] ; then
    ...
    export LSCL_SCRIPT_TO_RUN_IN_PARALLEL="lsclCreateIntegralFiles.sh"
    ...
    ${lsclScriptDir}/lsclTemplateScriptForm.sh ${lsclBasicArguments[@]:0:4} ${lsclDiaNumberFrom} ${lsclDiaNumberTo}
 else
    echo "${LSCL_SCRIPT_NAME}: Processing a single topology."
    ${lsclScriptDir}/lsclTemplateScriptForm.sh ${lsclBasicArguments[@]:0:4} ${lsclTopologyName}
fi
