...
# ./ShellScripts/lsclSlurmFireCreateLiteRedFiles.sh MyProject MyProject MyModel 1 clusterPartitions --fromto 1 all
# ./ShellScripts/lsclSlurmFireCreateLiteRedFiles.sh MyProject MyProject MyModel 1 clusterPartitions --mem 2500 --fromto 1 all --force
# ./ShellScripts/lsclSlurmFireCreateLiteRedFiles.sh MyProject MyProject MyModel 1 clusterPartitions --mem 2500 --fromto 1 all --force --nodes 2 --slices 1000
# ./ShellScripts/lsclSlurmFireCreateLiteRedFiles.sh MyProject MyProject MyModel 1 clusterPartitions --mem 2500 --fromto 1 all --clearlogs --nodes 2 --slices 1000
# ./ShellScripts/lsclSlurmFireCreateLiteRedFiles.sh MyProject MyProject MyModel 1 clusterPartitions --mem 1000 --fromto 1 all --nodes 2 --slices 5 --clearlogs


# Stop if any of the commands fails
set -e

export LSCL_SLURM_SCRIPT_NAME="lsclSlurmFireCreateLiteRedFiles"
...
export LSCL_CLUSTER_SCRIPT_NAME="lsclFireCreateLiteRedFiles.sh"
...
# echo "${lsclTasksAll[*]}"
  ...
  if [[ ${lsclTopoIndex} -lt 0 ]] ; then
      echo "${LSCL_SLURM_SCRIPT_NAME}: Failed to found the specified topology ${lsclTopoName} in the list."
      exit 1
  fi
